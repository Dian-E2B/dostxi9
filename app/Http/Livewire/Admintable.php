...
class Admintable extends DataTableComponent
{
    ...
    public function builder(?string $year = null): EloquentBuilder
    {
        ...
        $query = User::query()
            ->where('role', 'admin')
            ->whereNot('email', 'super@admin.com');
        return $query;
    }
            ...
            Column::make('Actions')
                ->label(
                    function ($row, Column $column) {
                        $delete = '<button data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Lock"  class="thisbutton btn-sm btn btn-primary" wire:click="delete(' . $row->id . ')"><i class="bi bi-person-fill-lock"></i></button>';
